...
getAllgens = result_bdd_request => {
    dataBase.query("SELECT gens.id,club_id AS clubId,nom,prenom,dettes.id as dettes FROM gens LEFT JOIN dettes on dettes.gens_id = gens.id", (error, response) => {
        if (error) {
...
getgensById = (selectedID, result_bdd_request) => {
    dataBase.query(`SELECT gens.id,club_id AS clubId,nom,prenom,dettes.id as dettes FROM gens LEFT JOIN dettes on dettes.gens_id = gens.id WHERE gens.id = ${selectedID}`, (error, response) => {
        if (error) {
            result_bdd_request(error);
        }
        if (response.length) {
...
creategens = (newgens, result_bdd_request) => {
    var club_id = newgens.clubId;
    club_id = club_id.replace('/api/v1/clubs/','');
    dataBase.query('INSERT INTO gens SET club_id = ?, nom = ?, prenom = ?', [club_id, newgens.nom, newgens.prenom],
        (error, response) => {
            if (error) {
                result_bdd_request(error);
            }
...
updategensById = (selectedID, selectedgens, result_bdd_request) => {
    dataBase.query(
        "UPDATE gens SET club_id = ?, nom = ?, prenom = ? WHERE id = ?",
