...
getAllclubs = result_bdd_request => {
    dataBase.query(`SELECT club.id,club.nom,club.email,gens.id as gens FROM club LEFT JOIN gens on gens.club_id = club.id`, (error, response) => {
        if (error) {
...
getclubById = (selectedID, result_bdd_request) => {
    dataBase.query(`SELECT club.id,club.nom,club.email,gens.id as gens FROM club LEFT JOIN gens on gens.club_id = club.id WHERE club.id = ${selectedID}`, (error, response) => {
        if (error) {
            result_bdd_request(error);
        }
        if (response.length) {
